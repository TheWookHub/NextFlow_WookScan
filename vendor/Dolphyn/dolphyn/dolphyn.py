...
def balance_DS(X, y, random_state=42):
    ...
    pos_IDs = list(y[y==1].index)
    neg_IDs = list(y[y==0].index)
    neg_IDs= random.sample(neg_IDs, size_smaller_group)
    pos_IDs= random.sample(pos_IDs, size_smaller_group)
    balancedIDs = list(pos_IDs + neg_IDs)
    y_bal = y.loc[list(balancedIDs),]
    X_bal = X.loc[list(balancedIDs),]
...
def initEpiPredictor(epi_size = 15, n_jobs=6, random_state=42, testset_size = 0, base_dir=None):
    feat,labs = getPEDSTrainingSet(epi_size, base_dir=base_dir)
    X=feat
    y=labs
...
def getPEDSTrainingSet(epi_size, base_dir=None):
    if base_dir is None:
        # Automatically locate the folder where the current file (dolphyn.py) is located
        base_dir = os.path.join(os.path.dirname(__file__), "trainingdata")
    
    feature_file = os.path.join(base_dir, f"peds_features_{epi_size}mer.csv")
    label_file = os.path.join(base_dir, f"peds_labels_{epi_size}mer.csv")
    ...
    feat = pd.read_table(feature_file, sep=",", index_col = 0)
    labs = pd.read_table(label_file, sep=",", index_col = 0)
...
def kmer_features_of_protein_new(seq, k):   
    feat = {}
    l = len(seq) 
    ...
    for i in range(0, l):
        aa = seq[i]
        feat.loc[max(0, i - k + 1):min(lk1, i), aa] += 1
        for f in AA_FEAT[aa]:  
            feat.loc[max(0, i - k + 1):min(lk1, i), f] += 1
    rangelen = l-1
    for i in range(0, rangelen):
        double = seq[i:i+2]
        feat.loc[max(0, i - k + 2):min(lk1, i), double] += 1
        diam_double = diam_seq[i:i+2]
        feat.loc[max(0, i - k + 2):min(lk1, i), "d_" + diam_double] += 1

    return feat

    
def kmer_features_of_protein_old(seq, k):   
    feat = {}
    l = len(seq) 
    diam_seq = get_diam(seq)
    lk1 = l-k+1
...
def findEpitopes(protein_seq_file, epitile_size, testrun = 0, epitope_probability_cutoff = 0.5, base_dir=None):
    count = 0
    global_epitopes = {}   
    ep, _ = initEpiPredictor(epitile_size, base_dir=base_dir)    
    ...
    with open(protein_seq_file, "r") as org:
        for line in org.readlines():
            ...
            if line.startswith(">"):
                ...
                prot_15mer_feat = kmer_features_of_protein_new(pseq,epitile_size)
                ...
                while len(epitope_proba_atpos)>0:
                    ...
                    if min_epi_proba < epitope_probability_cutoff:
                        ...
                        if epi_seq not in global_epitopes:
                            global_epitopes[epi_seq] = {"probability":epi_proba,"proteins":[],"start_pos":[], "origin_sequences": [pseq]}  
                        else: 
                            if isinstance(global_epitopes[epi_seq]["origin_sequences"], str):
                                global_epitopes[epi_seq]["origin_sequences"] = [global_epitopes[epi_seq]["origin_sequences"]]
                                
                            if pseq not in global_epitopes[epi_seq]["origin_sequences"]:
                              global_epitopes[epi_seq]["origin_sequences"].append(pseq)  
                                #print(f"APPEND: epitope={epi_seq}, origin_sequences={global_epitopes[epi_seq]['origin_sequences']}") 
...
def saveGlobalEpitopes(global_epitopes, filename):
    ...
    with open(filename,"w") as f:
        f.write(json.dumps(global_epitopes, cls=NpEncoder, indent=2))
