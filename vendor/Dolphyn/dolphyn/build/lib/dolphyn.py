...
def balance_DS(X, y, random_state=42): # done
...
def train_test_split_wildtypes(X, y, test_size, random_state=42): # done
    wildtypeIDs = set([item[0] for item in X.index.str.split("_")])
    random.seed(random_state)
...
def initEpiPredictor(epi_size = 15, n_jobs=6, random_state=42, testset_size = 0): # done
...
def getPEDSTrainingSet(epi_size): # done
...
def get_diam(aa_seq): # done
    diam = ""
    for aa in aa_seq:
        diam += AA_DIAM[aa]        
    return(diam)

def kmer_features_of_protein(seq, k):   # done
    feat = {}
    l = len(seq) 
...
def findEpitopes(protein_seq_file, epitile_size, testrun = 0, epitope_probability_cutoff = 0.5): # done
    count = 0
    global_epitopes = {}   
...
def saveGlobalEpitopes(global_epitopes, filename): # done
    import json   
...
def readGlobalEpitopes(filename): # done
    import json
