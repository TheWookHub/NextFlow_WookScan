...
def compute_edge_restrictions(): # done
    prefix_restrictions = []
    suffix_restrictions = []
...
def get_edge_restrictions(write_file=True): # done
    ...
    # edge_restrictions = edge_restrictions.map(eval).iloc[:, 0].to_dict()
    edge_restrictions = edge_restrictions.map(eval).iloc[:, 0].to_dict()


def has_no_restricted_sequences(nuc_seq): # done
...
def update_unconverted_oligos_file(unconverted_oligos): # done
    current_unconverted = read_unconverted_sequences()
    
    # pd.concat([...]) 将新未转换的 unconverted_oligos[['oligo_aa_sequence']] 和 current_unconverted 拼接在一起，确保之前的未转换记录不会丢失。
    pd.concat([current_unconverted, unconverted_oligos[['oligo_aa_sequence']]]).to_csv(UNCONVERTED_SEQUENCES_FILE)


def initializer(edge_restriction_file): # done
    global EDGE_RESTRICTION_FILE
    EDGE_RESTRICTION_FILE = edge_restriction_file


def aa_to_nuc(oligos_aa_sequences): # done
