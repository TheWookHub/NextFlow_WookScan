...
workflow BIPS_THEN_DOLPHYN {
    ...
    // }
			...
			if (params.mode == "bips_then_dolphyn" || params.mode == "bips_only") {
                    ...
                    ch_bips_full_output_dir = RUN_BIPS_INITIAL.out.bips_full_outputs_dir
...
process RUN_BIPS_INITIAL {
    ...
    tuple val(sample_id), path ("${bips_code_dir.baseName}/Data/Output"), emit: bips_full_outputs_dir
    ...
    # --- Prepare BIPS input ---
    INPUT_FILE_BASENAME="\$(basename -- "${viral_seq_file.name}" | cut -f 1 -d '.')"
    TARGET_BIPS_INPUT_FILENAME="\${INPUT_FILE_BASENAME}.fa"
    TARGET_BIPS_INPUT_PATH="\${BIPS_INPUT_DIR}/\${TARGET_BIPS_INPUT_FILENAME}"

    cp "${viral_seq_file.name}" "\${TARGET_BIPS_INPUT_PATH}"
    echo "Copied input to BIPS: \${TARGET_BIPS_INPUT_PATH}"
