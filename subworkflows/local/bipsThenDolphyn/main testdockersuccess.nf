...
workflow BIPS_THEN_DOLPHYN {
    ...
    // log.info("${ch_oligos_csv_for_dolphyn_only}")
    ...
    //         log.info "Mode 'dolphyn_oligo_only': Attempting Channel.fromPath with params.input_oligos_fasta: '${params.input_oligos_fasta}'"
    ...
    //         log.info "Mode 'dolphyn_oligo_only': Attempting Channel.fromPath with params.input_bips_oligos_csv: '${params.input_bips_oligos_csv}'"
    ...
    // }
			...
			if (params.mode == "bips_then_dolphyn" || params.mode == "bips_only") {
                    ...
                    //def ch_bips_full_output_dir = RUN_BIPS_INITIAL.out.bips_full_outputs_dir.map { single_dir  -> tuple(batch_id, single_dir) }
			...
			// ========================
            // We don't need an `if (params.mode == 'dolphyn_oligo_only')` block here anymore,
            ...
            //BIPS_CSV_TO_FASTA(ch_oligos_csv_for_dolphyn_only)
            // def ch_fasta_from_csv_conversion = BIPS_CSV_TO_FASTA.out.oligos_fasta
			...
			// if (params.mode == "dolphyn_oligo_only") {
            //     //def ch_oligos_csv_for_dolphyn_only = 

            //     //use 'when' here to seperate ch_oligos_fasta_for_dolphyn_only and ch_oligos_csv_for_dolphyn_only!
            ...
            //                             .mix(ch_oligos_fasta_for_dolphyn_only) // Add results from dolphyn_oligo_only (direct fasta)
            //                             .mix(ch_fasta_from_bips_csv) // Add results from dolphyn_oligo_only (csv conversion)
			...
			//     .map { count -> count > 0 }
			//boolean run_dolphyn_branch = (params.mode == "bips_then_dolphyn" || params.mode == "dolphyn_oligo_only") && !ch_fasta_ready_for_dolphyn.isEmpty().toBlocking().first()
    ...
    emit:
        ...
        //bips_whole_outputs_dir = ch_bips_full_output_dir
...
process RUN_BIPS_INITIAL {
    ...
    // Use a fixed name for the batch output directory
    // publishDir path: "${params.outdir}/bips_batch_run_outputs",
    //            mode: 'copy',
    //            overwrite: true,
    //            saveAs: { it.name } // Publish contents directly

    // publishDir = {
    //     [
    //         path: "${params.outdir}/bips_batch_run/bips_run_outputs", // Fixed path for the batch
    //         mode: 'copy',
    //         overwrite: true,
    //         pattern: "${bips_code_dir.baseName}/Data/Output/*"
    //     ]
    // }
    publishDir = [
        ...
        mode: 'copy',
        overwrite: true,
        //pattern: "bips_full_output" // Hardcode the vendored directory name
    ]
    
    

    // publishDir = [
    //         "${params.outdir}/bips_batch_run_outputs",
    //         mode: 'copy',
    //         overwrite: true,
    //         pattern: "BuildPhIPSeqLibrary/Data/Output/*"   //"bips_final_outputs" // Target a fixed, known directory name
    // ]

    // publishDir from: bips_full_output_dir_for_publishing,
    //            path: { "${params.outdir}/bips_batch_run_outputs" },
    //            mode: 'copy',
    //            overwrite: true
    ...
    path "bips_full_output"  //, emit: bips_full_outputs_dir
    ...
    def full_output_container_name = "bips_full_output" // A fixed name for the full output dir

    """
    echo "RUN_BIPS_INITIAL for a batch of files"
    echo "Staged BIPS code directory: ${bips_code_dir}"

    # --- KEY FIX: Create a physical copy with a NEW NAME ---
    # We will operate on a new directory called 'BIPS_WORKDIR'
    BIPS_WORKDIR="BIPS_WORKDIR"
    echo "Creating a physical copy of BIPS code in '\$BIPS_WORKDIR'..."
    cp -rL "${bips_code_dir}" "\$BIPS_WORKDIR"
    echo "Physical copy created."

    # 2. Define ABSOLUTE paths inside the work directory for I/O
    ABS_INPUT_DIR="\$(pwd)/\$BIPS_WORKDIR/Data/Input"
    ABS_OUTPUT_DIR="\$(pwd)/\$BIPS_WORKDIR/Data/Output"
    CONFIG_FILE="\$BIPS_WORKDIR/BuildPhIPSeqLibrary/config.py"

    # 3. Clean and prepare these I/O directories
    mkdir -p "\$ABS_INPUT_DIR" "\$ABS_OUTPUT_DIR"
    find "\$ABS_INPUT_DIR" -mindepth 1 -type f ! -name 'README.md' -delete
    find "\$ABS_OUTPUT_DIR" -mindepth 1 -type f ! -name 'README.md' -delete
    # --- End Cleaning Step ---
    ...
    for file in ${file_list}; do
        ...
        cp "\$file" "\$ABS_INPUT_DIR/"
        echo "  Copied \$file"
    done
    echo "All input files copied to \$ABS_INPUT_DIR"

    # 5. Dynamically modify config.py to use these absolute paths
    # This forces BIPS to work within the Nextflow work directory.
    echo "Modifying BIPS config file: \$CONFIG_FILE to use absolute paths..."
    # Using '#' as a delimiter for sed to handle '/' in paths
    sed -i "s#^INPUT_DIR = .*#INPUT_DIR = '\$ABS_INPUT_DIR'#" "\$CONFIG_FILE"
    sed -i "s#^OUTPUT_DIR = .*#OUTPUT_DIR = '\$ABS_OUTPUT_DIR'#" "\$CONFIG_FILE"
    echo "BIPS config modified."


    # --- Run BIPS ---
    cd "\$BIPS_WORKDIR"
    echo "Executing BIPS in directory: \$(pwd)"
    ...
    echo "--- BIPS STDOUT ---"
    cat "\$BIPS_WORKDIR/bips_stdout.log"
    echo "--- BIPS STDERR ---"
    cat "\$BIPS_WORKDIR/bips_stderr.log" >&2
    ...
    BIPS_RAW_OUTPUT_DIR="\$BIPS_WORKDIR/Data/Output"
...
process PUBLISH_BIPS_RESULTS {
    ...
    tag "Publishing BIPS full results for $batch_id"
    ...
    input:
    // It receives the batch_id and the Path object for the directory to be published
    tuple val(batch_id), path(bips_output_dir)

    output:
    // This process doesn't need to produce an output for other processes,
    // but we can emit a signal file to indicate completion.
    path "${batch_id}.bips_published.txt"

    script:
    // Define the final target directory path
    def final_path = "${params.outdir}/${batch_id}"

    """
    echo "Publishing BIPS full results for sample ${batch_id}"
    ...
    cp -a "${bips_output_dir}"/* "${final_path}/"

    echo "Publishing complete. Final contents of ${final_path}:"
    ls -lAh "${final_path}"

    # Create a small file to signal that publishing is done
    echo "Published on \$(date)" > "${batch_id}.bips_published.txt"
    """
}


# --- FINAL PART of the script (MODIFIED) ---

    # 1. Create a dedicated directory for publishing
    mkdir -p bips_final_outputs

    # 2. Copy ALL of BIPS's results into this new directory
    echo "Copying all BIPS results to 'bips_final_outputs' for publishing..."
    cp -rL "${bips_staged_name}/Data/Output/"* bips_final_outputs/

    # 3. Copy the two key files needed by downstream channels to the work dir root
    if [ -f "bips_final_outputs/${oligos_out_name}" ]; then
        cp "bips_final_outputs/${oligos_out_name}" .
        echo "Copied ${oligos_out_name} for downstream processes."
    else
        echo "ERROR: BIPS output ${oligos_out_name} not found in results!" >&2; exit 1
    fi
    if [ -f "bips_final_outputs/${barcoded_out_name}" ]; then
        cp "bips_final_outputs/${barcoded_out_name}" .
        echo "Copied ${barcoded_out_name} for downstream processes."
    else
        echo "ERROR: BIPS output ${barcoded_out_name} not found in results!" >&2; exit 1
    fi
